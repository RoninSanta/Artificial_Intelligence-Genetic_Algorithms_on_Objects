...
class Genome():
    ...
    def get_gene_spec():
        gene_spec = {"link-shape": {"scale": 1},
                     ...
                     "segment_length": {"scale": 2},
                     "segment_width": {"scale": 0.5},
                     "joint_friction": {"scale": 1},
                     "muscle_strength": {"scale": 1},
                     "joint_range": {"scale": 1}
                     }
        ind = 0
    ...
    def genome_to_links(gdicts):
        ...
        for gdict in gdicts:
            ...
            assert parent_ind < len(parent_names), "genome.py: parent ind too high: " + str(
                parent_ind) + "got: " + str(parent_names)
            ...
            link = URDFLink(name=link_name,
                            ...
                            segment_length=gdict["segment_length"],
                            segment_width=gdict["segment_width"],
                            joint_friction=gdict["joint_friction"],
                            muscle_strength=gdict["muscle_strength"],
                            joint_range=gdict["joint_range"],
                            )
            links.append(link)
